...
class DomReceiver {
  constructor({
    ...
    this.receive = createRemoteChannel({
      mount: serializedChildren => {
        ...
        AndroidMount(JSON.stringify(serializedChildren))
        const {
          bound,
          children
        } = this;
      ...
      insertChild: (id, index, child) => {
        ...
        AndroidInsertChild(JSON.stringify(child));
        const node = this.deserialize(child);
  ...
  deserialize(serialized) {
    ...
    if ('text' in serialized) {
      ...
      if (elementType == null) {
        throw new Error(`Can’t create component of type ${JSON.stringify(serialized.type)} because it does not map to a custom element.`);
      }
...
function AndroidResponse(str){
  Android.response(str);
}

function AndroidMount(str){
  Android.mount(str);
}

function AndroidInsertChild(str){
  Android.insertChild(str);
}

function AndroidRemoveChild(str){
  Android.removeChild(str);
}
